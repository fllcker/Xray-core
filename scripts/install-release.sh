...
# Installs the fllcker fork of Xray-core.
#
# Derived from XTLS/Xray-install (install-release.sh), which is licensed under
# the GNU General Public License v3.0. This modified copy stays under GPL-3.0,
# separately from the rest of this repository, which is MPL-2.0. The two do not
# interfere: this is a standalone shell script, not part of the binary.
#
# Original: https://github.com/XTLS/Xray-install
#
# The only change is where releases come from. Upstream hardcodes
# XTLS/Xray-core, so a fork cannot be installed with it; here the repository is
# a variable.

# The GitHub repository releases are pulled from. Set it to your own fork.
# Override without editing the file:
#   GITHUB_REPO=you/your-fork bash install-release.sh install
GITHUB_REPO=${GITHUB_REPO:-fllcker/xray-core}
...
get_latest_version() {
  ...
  if [[ -z "$RELEASE_LATEST" ]]; then
    if grep -q "API rate limit exceeded" "$tmp_file"; then
      ...
      echo "Welcome bug report:https://github.com/${GITHUB_REPO}/issues"
    fi
    "rm" "$tmp_file"
    exit 1
  fi
  ...
  for i in "${!releases_list[@]}"; do
    ...
    url_zip="https://github.com/${GITHUB_REPO}/releases/download/${releases_list[$i]}/Xray-linux-$MACHINE.zip"
...
download_xray() {
  local DOWNLOAD_LINK="https://github.com/${GITHUB_REPO}/releases/download/${INSTALL_VERSION}/Xray-linux-${MACHINE}.zip"
